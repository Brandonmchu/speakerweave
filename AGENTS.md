...
| AI | Replace `api/services/ai_triage.py:_call_anthropic()` and `api/services/slack_agent.py:_call_anthropic()` plus their model constants/schema translation. | Triage still returns a complete, labelled heuristic result when AI is absent or fails; Slack tools still dispatch through organization-scoped `services.integration_api`; human decisions remain authoritative. |
...
- **Slack:** import `api/slack_manifest.json`; signed events enter through `api/routes/slack_routes.py` and agent/tool behavior lives in `api/services/slack_agent.py`.
